...
#### 1.1.  The following software should be installed:

    * `docker`;
    * `jq`.

#### 1.2.  This instruction was checked on:
...
#### 1.3. *IMPORTANT!* The following conditions must be met:
...
#### 1.4.  The following Docker images are used in this instruction:
...
#### 1.5.  Be sure subnet `192.168.10.0/24` is not used on your local machine and there is no Docker network named `op-local`.

#### 1.6.  All the docker commands in this instruction are executed from the superuser using the `sudo` prefix.


## 2. Generate P2P keys and IDs for nodes

#### 2.1.  Generate or select some 32-bit (64 hex chars) private keys, like:
...
#### 2.2. For each private key generate the appropriate P2P ID with the following commands:
...
#### 3.1.  Head over to the [docker/prerequisite](./docker/prerequisite) directory of this repository:
...
#### 3.2. Replace files `genesis.json` and `rollup.json` with ones your have.


#### 3.3. Open file [envfile](./docker/prerequisite/envfile). Replace its content with appropriate data and data you've got previously, like:
...
#### 4.1. Head over to the [docker](./docker) directory of this repository:
...
#### 4.2. If you want to change or completely remove the forwarding ports from the containers to your local machine edit `ports` sections in all `docker-compose.yaml` files in the directory.
...
#### 4.3. Execute the [init.sh](./docker/init.sh) script to initialize all the needed files to run the containers:
    ```bash
    sudo ./init.sh
    ```
...
#### 4.4. Execute the [up.sh](./docker/up.sh) script to up and run all the containers:
    ```bash
    sudo ./up.sh
    ```


#### 4.5. Be sure that all containers are working using the command:
```bash
...
#### 4.6. To stop and remove all the containers (but not their data) execute the [down.sh](./docker/down.sh) script:
```bash
sudo ./down.sh
```
...
#### 4.7. To delete the data of all containers after stopping them execute the [clear.sh](./docker/clear.sh) script:
```bash
sudo ./clear.sh
```
...
#### 5.1. The nodes of the network can be accessed through their Docker internal IP addresses and ports or through the forwarded ports if you configured them (see the `docker-compose.yml` files). E.g. the default RPC URL of nodes are listed in the table bellow:
...
#### 5.2. You can check that blocks are being produced and can be accessed through each node with the following script:
...
#### 5.3.  Information about getting native tokens (ETH) inside the newly created L2 network see [here](single-node-no-docker.md#9-use-the-newly-created-l2-network).
...
#### 6.3. Enable metrics in docker-composer.yml
...
6.4.1.  Get the number of docker GID by running the following command in the terminal:

   ```
   $(stat -c '%g' /var/run/docker.sock)
   ```

6.4.2. Set the outputted number to variable `CW_OP_TELEGRAF_DOCKER_ID` in file `docker/prerequisite/envfile`
...
#### 7.3. Running Environment with Debug Mode
...
#### 7.4. Stop Environment with Debug Mode
